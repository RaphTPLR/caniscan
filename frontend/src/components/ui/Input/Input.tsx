...
export const Input = forwardRef<HTMLInputElement, InputProps>(
    ({ label, name, type = 'text', error, rightIcon, className = '', ...props }, ref) => {
        ...
        const disabledClasses = props.disabled
            ? 'bg-gray-50 text-gray-500 placeholder-gray-400 border-gray-200 cursor-not-allowed'
            : '';
            ...
            <div className="relative w-full">
                {label && (
                    <label htmlFor={name} className="mb-1 block text-sm font-medium text-gray-700">
                        ...
                        {props.disabled && <Lock className="ml-1 inline h-3 w-3 align-text-bottom text-gray-400" />}
                    </label>
                )}
